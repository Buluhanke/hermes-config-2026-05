...
def cmd_stats(days: int = 7):
    ...
    total_pt = sum(r["prompt_tokens"] or 0 for r in rows)
    total_ct = sum(r["completion_tokens"] or 0 for r in rows)
    ...
    print(f"延迟:     avg={avg_lat:.0f}ms  p95={p95_lat:.0f}ms")
    print(f"Token:    输入={total_pt:,}  输出={total_ct:,}")
...
def cmd_providers():
    ...
    print(f"{'Provider':<20} {'Model':<30} {'次数':>6} {'延迟':>8} {'Prompt':>10} {'Completion':>10}")
    print("-" * 90)
    for r in rows:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Hermes LLM 可观测性")
