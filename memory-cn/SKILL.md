...
# Memory-CN — Hermes 中文记忆系统优化 (v2.1)

## 架构现状（2026-07-18 v2.1）

Hermes 记忆系统是 **三层架构**：

| 层 | 组件 | 路径 | 用途 |
|----|------|------|------|
| P0 | `MEMORY.md` | `~/.hermes/memories/` | 核心偏好/高频规则 |
| P1 | `fact_store` (FTS5) | `~/.hermes/memory_store.db` | 经验/教训/规则沉淀 |
| P2 | Mnemosyne (向量+SQLite) | `~/.hermes/mnemosyne.db` | 语义记忆/偏好（未激活） |

**2026-07-18 确认**：`memory.provider = holographic`，使用 `memory_store.db` 的 FTS5 检索。Mnemosyne 已装但未启用（config 里没有配置）。
...
```bash
# fact_store 基本统计（正确路径）
sqlite3 ~/.hermes/memory_store.db "SELECT COUNT(*) FROM facts;"
sqlite3 ~/.hermes/memory_store.db "SELECT COUNT(*), AVG(trust_score) FROM facts;"

# retrieval_count 分布（2026-07-18 新增诊断）
sqlite3 ~/.hermes/memory_store.db "SELECT COUNT(*) FROM facts WHERE retrieval_count = 0;"
sqlite3 ~/.hermes/memory_store.db "SELECT COUNT(*) FROM facts WHERE retrieval_count >= 1;"

# 低信任/过期条目
sqlite3 ~/.hermes/memory_store.db "SELECT fact_id, trust_score, retrieval_count FROM facts WHERE trust_score <= 0.3;"

# 无tags条目（搜索命中率杀手）
sqlite3 ~/.hermes/memory_store.db "SELECT fact_id, LENGTH(tags) FROM facts WHERE tags IS NULL OR tags = '';"
```
...
|------|--------|----------|
| 活跃条数 | > 50 | < 20 时需激活 |
| 平均 trust_score | ≥ 0.4 | < 0.3 时需衰减 |
| retrieval_count > 0 | > 0 | 全 0 意味着检索链路断路（见下方排查） |
| 无tags条数 | 0 | > 5 时需补打 |

**retrieval_count 全 0 排查**（2026-07-18 实测）：

1. 确认 `config.yaml → memory.provider = holographic`
2. 重启 gateway 后检查日志：`grep holographic ~/.hermes/logs/gateway.log`
3. 测试检索：`plugins/memory/holographic/retrieval.py` 的 `FactRetriever.search()` 是否更新计数
4. 独立验证脚本见 `abcd-learner/references/holographic-memory-debug-20260718.md`
...
**2026-07-18 确认**：Mnemosyne (`~/.hermes/mnemosyne.db`) 已装，但 `config.yaml` 未配置，不是当前 provider。

如需切换：
```bash
# 在 config.yaml memory: 下加
provider: mnemosyne
```

当前活跃 provider 是 `holographic`，走 `memory_store.db` 的 FTS5。
