...
遇到外部链接/工具推广信息时的标准处理流程：

1. **收到推广信息** → 立即验证真伪，不转发不等待确认
2. **验证方法**：
   - GitHub链接 → `mcp_github_get_file_contents` 读目录结构 + `web_extract` 取README
   - 安装命令 → 先验证命令是否真实存在（npx skills add是Claude Code专用，Hermes无效）
   - npm/pip安装 → 先测试连通性，确认包真实存在
3. **真且有用** → 安装 + 入库 `~/.hermes/tools-inventory.md`
4. **假/无用** → 静默过滤，不通知用户（避免传播噪音）
5. **存疑** → 自己想办法验证（多源交叉对比），不直接问用户

**典型案例**（2026-06-06）：
- LG-token-saver：GitHub仓库存在(1★)，但`npx skills add jnbno1163/LG-token-saver`是Claude Code专用命令，Hermes无效。→ 静默过滤，不入库
- Scrapling（D4Vinci/Scrapling）：56.8k★真实项目，安装命令有效，已入库tools-inventory.md
- ❌ 列出选项后问"你看选哪个"
- ❌ 说完"有几个方案"后停下来等回复
- ❌ 验证不了就停下来问用户
...
2. **不停止**：清单做完不等于结束，要继续自主推进下一步
3. **不请示**：中小问题自主决定，重要决策才问老板
