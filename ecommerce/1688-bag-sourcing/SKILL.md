...
**D. macOS 无 `timeout` 命令**：后台跑长任务别包 `timeout 1500 ...`，会 `command not found`。直接裸跑 + `background=true` + `notify_on_complete`（macOS native no timeout cmd）。

**E. 输出铁律（用户风格，最高优先）**：先给结论 + 链接 + 价格表，再补必要说明。不要先铺原理/坑位/分析墙。表格列：商品ID | 规格 | 单价 | 白边 | 12丝 | 是否目标。

---

## 坑位 F-I（2026-09-01 新增）

**F. 拉环袋 ≠ 自封袋，品类过滤不能混用**：本次「磨砂拉环袋 18*15cm」跑了多轮全0——根因是 `BAG_SIG` 把「拉环袋」当成「非自封袋」跳过了。拉环袋（带提手）和普通自封袋是**两个品类**，搜词必须用：`磨砂拉环袋`、`拉链袋`、`磨砂手提袋`。

**G. 用户链接优先于搜索（最高优先铁律）**：用户给了 `791933298070`（星喆 ¥0.15），但我先搜了 59 个候选才去看链接——这家根本不在公开池里（个性化 Session）。**收到用户链接 → 直接核验，不要先搜**。

**H. CDP 登录墙宕机时的三重兜底**：CDP Chrome 登录态丢 → 详情页跳 taobao.com → 全0。备用通道：
1. **MCP** `mcp__alibaba_1688_scraper__get_product_detail_and_price`（最快，约70%成功率）
2. **desktop_preview**（Hermes 桌面 app 内置通道，能穿透登录墙）
3. **AppleScript** 读默认 Chrome（最慢但最稳）
工作流：用户给链接 → 先试 MCP → MCP 失败用 desktop_preview 兜底。

**I. 搜候选池 ≠ 货源池，0命中 ≠ 没货**：本次搜「磨砂拉环袋 18*15cm」59 个候选全近似尺寸（18*10、20*15），18*15 精确命中0。但用户链接里星喆确实在卖。说明该尺寸在品类里是非主流刀模。**不能因搜0命中就下结论**。
