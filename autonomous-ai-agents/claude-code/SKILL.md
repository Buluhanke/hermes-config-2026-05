...
{
  "permissions": {
    "allow": ["Bash(npm run *)", "Read", "Edit", "WebSearch"],
    "ask": ["Write(*.py)", "Bash(git push *)"],
  ...
  "includeErrors": true,
  "verbose": false,
  "legacyFileEditWarning": false,
  "autoCompliance": false,
  "autoCompact": false,
  "disableGlobalRules": false,
  "disableProjectRules": false,
  "disablePersonalRules": false,
  "compactThreshold": 0.85,
  "autoContextPruning": false,
  "lowMemory": false
}
```
...
| Mode | Behavior |
|------|----------|
| `default` | Ask before potentially destructive operations |
| `dontAsk` | Never ask (requires `allow` list to permit anything) |
| `bypassPermissions` | Auto-approve all operations (dangerous) |
| `acceptEdits` | Auto-approve file edits without asking |
| `plan` | Enter Plan mode before any tool use |
| `auto` | Claude decides when to ask based on risk |

### Model Selection
| Model | Use Case |
|-------|----------|
| `sonnet` / `claude-sonnet-4-6` | Default, balanced |
| `opus` / `claude-opus-4-5` | Complex reasoning, large refactors |
| `haiku` / `claude-haiku-4` | Fast, simple tasks |
| `sonnet-4-7-2026` | Latest stable |
...
## PR Review Automation Flow

### Automated PR Review Pipeline

```bash
# Full automated review workflow
terminal(command="cat << 'EOF' > /tmp/pr-review-pipeline.sh
#!/bin/bash
set -e
...
cd \"$REPO\"
gh pr checkout \"$PR_NUM\" 2>/dev/null || gh pr view \"$PR_NUM\" --repo \"$REPO\" --json url | jq -r '.url'

echo \"=== Running automated review ===\"
${CLAUDE_CMD} \"Review PR #${PR_NUM} for:
...
cat /tmp/pr-review-result.json
EOF
chmod +x /tmp/pr-review-pipeline.sh", timeout=10)
```

### CI/CD PR Review Integration
...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "gh pr comment $PR_NUMBER --body 'Claude review complete. Reviewer: @reviewer'"
      }]
    }]
  }
}
...
Claude produces structured review comments. Parse with:
```bash
# Extract findings by severity
cat review-result.json | jq '.result' | jq -r 'split(\"\\n\") | .[] | select(startswith(\"##\"))'

# Get action items
cat review-result.json | jq '.result' | jq -r 'split(\"\\n\") | .[] | select(startswith(\"- [ ]\"))'
```

## Hermes Collaboration Workflow

### Direct Integration Pattern
```bash
# Hermes delegates a coding task to Claude Code
terminal(command="claude -p \"Implement the user authentication module\" \
  --workdir \"/path/to/project\" \
  --output-format json \
  --max-turns 20 \
  --allowedTools \"Read,Edit,Write,Bash\" \
  > /tmp/auth-module-result.json", timeout=180)

# Hermes reads the result and continues
terminal(command="cat /tmp/auth-module-result.json | jq '{session_id, result: .result[:500], cost: .total_cost_usd}'", timeout=10)
```

### Hybrid Orchestration (Hermes + Claude)
```bash
# Step 1: Hermes plans the task breakdown
# Step 2: Claude implements each component
# Step 3: Hermes reviews and coordinates

# Example: Multi-component implementation
terminal(command="claude -p \"Implement a REST API with these components:
1. User model (src/models/user.py)
2. Auth endpoints (src/api/auth.py)  
3. Database migrations (migrations/)
4. Unit tests (tests/)
Use FastAPI, SQLAlchemy, pytest.\" \
  --workdir \"/project\" \
  --allowedTools \"Read,Edit,Write,Bash\" \
  --max-turns 30", timeout=300)
```

### Session Coordination
```bash
# Hermes creates a session and hands off to Claude for a complex task
terminal(command="SESSION_ID=$(claude -p \"Start implementing the payment gateway\" \
  --session-id \"$(uuidgen)\" \
  --output-format json \
  --max-turns 50 | jq -r '.session_id')
echo \"Session: $SESSION_ID\"
# Store for later resumption
echo $SESSION_ID > /tmp/payment-session-id", timeout=60)

# Later, resume and continue
terminal(command="claude -p \"Continue with the webhook handlers\" \
  --resume $(cat /tmp/payment-session-id) \
  --max-turns 20", timeout=120)
```

### Teammate Mode (Multi-Agent)
```bash
# Claude with subagent orchestration
terminal(command="claude -p \"Use the following agents to build this feature:
- @backend-dev: Implements API endpoints
- @tester: Writes tests alongside code
Coordinate between them.\" \
  --teammate-mode tmux \
  --workdir \"/project\"", timeout=300)
```

### Status Reporting Back to Hermes
```bash
# Capture Claude's work summary
terminal(command="tmux capture-pane -t claude-work -p -S -100 | tail -50", timeout=10)

# Get session cost and stats
terminal(command="claude -p \"/cost\" --continue --max-turns 1", timeout=30)
```

## Code Explanation Generation

### Generate Explanations via Print Mode
```bash
# Explain a single file
terminal(command="claude -p \"Explain this code in detail, covering:
...
# Explain complex logic with context
terminal(command="claude -p \"Explain this function, its inputs, outputs, and side effects.
Also explain the algorithm used.\" \
  --workdir \"/project/src\" \
  --allowedTools \"Read\" \
  --max-turns 3", timeout=45)
```

### Structured Code Documentation
```bash
# Generate docstring for a function
terminal(command="claude -p \"Write a Google-style docstring for this function.
Include Args, Returns, Raises, and Example sections.\" \
  --workdir \"/project\" \
  --allowedTools \"Read,Edit\" \
  --max-turns 2", timeout=30)

# Generate README for a module
terminal(command="claude -p \"Generate a comprehensive README.md for this module.
Include: Overview, Installation, Usage examples, API reference, and Troubleshooting.\" \
  --workdir \"/project/src/module\" \
...
terminal(command="claude -p \"Analyze the architecture of this codebase.
Create a diagram description and explain:
...
terminal(command="claude -p \"Add inline comments explaining the complex parts of this code.
...
Claude auto-detects language from file extensions:
| Extension | Language |
|-----------|----------|
| `.py` | Python |
| `.ts` / `.tsx` | TypeScript |
...
terminal(command="claude -p \"Generate docstrings for all functions in this Python module.
Use Google style.\" \
  --workdir \"/project\" \
  --allowedTools \"Read,Edit\" \
  --max-turns 10", timeout=90)

# TypeScript JSDoc generation  
terminal(command="claude -p \"Add JSDoc comments to all exported functions.
