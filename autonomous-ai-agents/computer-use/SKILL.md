...
| Chrome extension toolbar popup won't respond to AX/click | Chrome extension popup windows (opened by clicking the toolbar icon) are NOT real browser tabs — they exist outside the CDP tab protocol and AX accessibility tree. CDP can see the extension's background service worker but cannot open or click the popup itself. The service worker only wakes when the user manually clicks the extension icon in Chrome's toolbar. Workaround: use CDP to navigate tabs, use computer_use to click the toolbar icon if it lands in an accessible AX window, or ask the user to click the extension icon once — the popup then stays open for the session and the relay auto-connects. |
