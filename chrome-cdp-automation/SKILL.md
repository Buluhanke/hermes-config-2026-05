...
| Grok | textarea×1 | yes (xAI account) | ⚠️ partial | Login redirects to accounts.x.ai; needs cookies in profile. **2026-06-03: Tiptap wrapper means `__reactProps` is on `t.parentElement` not the textarea itself. `Input.insertText` fails to populate; use React fiber parent-props path with `onCompositionEnd` first. Send button: `button[aria-label="提交"]`. Reply in `body.innerText` after ~60-90s (xAI Reasoning slow).** |
| ChatGPT | contenteditable (ProseMirror) | yes | ⚠️ ProseMirror | **2026-06-03: `Input.insertText` works for `#prompt-textarea` (cleanest path — focus first, then `Input.insertText`, text appears in `<p data-placeholder="...">`, send button enables). `ta.value=` does NOT work (ProseMirror controlled component). Send via Enter or click `button[data-testid="send-button"]`. Reply readable via `article[data-message-author-role="assistant"]:last-of-type` or AX tree on a fresh tab.** |
