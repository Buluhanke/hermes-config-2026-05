...
**进化方向铁律（2026-06-07 用户再次拍板强化）**：

> 进化 = 先用当下 → 搜罗更强的免费替代品 → 替换
> 修补 = 在坏掉的框架里不停找错 → 不算进化

**两种模式的本质区别**：

| | 进化（Replacement） | 修补（Patch） |
|--|---|---|
| 方向 | 搜罗 → 发现更好的 → 切换 | 调试 → 找bug → 修bug |
| 行为 | 搜索"best free X 2026" | 分析"为什么 X 又坏了" |
| 结果 | 能力跃升，质的改变 | 维持现状，边际改善 |
| 问法 | "有没有更强的工具？" | "这个工具为什么坏？" |
...
是"工具有问题"还是"配置/环境有问题"？
  ├─ 工具本身有缺陷 → 搜替代品（进化路径）
  ├─ 配置/集成/环境问题 → 先尝试修复（修补路径）
  └─ 修 3 次仍失败 → 停止修补，切换进化路径
```
...
**触发词**：
- "换思路" / "继续向前" / "别分析了" / "别修了" → 立即停手，进化搜索
- 搜索关键词：`best free [tool] 2026` / `[tool] alternative open source` / `[tool] vs [alternative]`

**反面教材（本 session 真实案例）**：
- CDP 9333 端口断了 → 花时间分析"为什么会断" → 修补方向
- 正确做法：端口断了是事实 → 直接搜"2026 年更好的浏览器自动化方案" → 找到 CloakBrowser → 发现无法替换（有架构原因）→ 记录为"互补工具"而非失败
- 关键：找到更强工具但无法替换 ≠ 失败，是正常的进化路径中的一站

**触发判断**：
- 用户说"进化"/"升级"/"替换" → 走进化路径
- 用户说"修"/"bug"/"坏了" → 先评估：能替换还是只能修补
- 修 3 次以上仍失败 → 切换思路，找替代方案

**正模式**：
1. 摸清当下真实状态（哪些模块在跑、哪些已坏）
2. 搜罗比当下更强的免费替代方案（不是找 bug，是找更好的）
3. 评估后直接替换（不列 3 个选项让用户选）
4. 验证新方案效果

**修补失败 → 立即切换进化模式**：
- 同一个问题修 3 次仍失败 → 停止修补，触发进化搜索
- 修的过程中发现根因是架构限制（无法绕开）→ 立即停止，直接搜替代方案
- 用户说"换个思路"/"继续向前"/"别分析了" → 立即停手，搜更好的替代工具
- **触发词**："换思路"、"继续向前"、"别修了"、"换个方向"、"别分析了" → 停止修补，进入进化搜索模式
- 进化搜索关键词：`best free X 2026` / `X alternative open source` / `X vs Y comparison`

**反模式（本次教训）**：
- 在 CDP 9333 断掉后，花了大量时间分析"为什么会断"
- 正确做法：先确认 Chrome 在跑但端口没监听（已经是现状），直接搜"2026 年更好的浏览器自动化方案有哪些"
- **不要在坏掉的框架里分析为什么坏，要去找更好的框架替换它**

**本次 CloakBrowser 进化实战（2026-06-07）**：
- 搜罗到 CloakBrowser（58项反检测补丁，业界最强）
- 安装成功，`pip install cloakbrowser` 直接装上（Python 3.14）
- 实测 bot.sannysoft.com → 反检测满分 ✅
- 但发现**无法替换 multi_ask_v3**：CloakBrowser 独立 profile = 无用户登录态；multi_ask_v3 用 CDP 控制用户 Chrome = 有登录态
- 两者是**互补工具**（爬虫/匿名测试 vs 已登录会话操控），不是替代关系
- **这次没有白学**：发现了 CloakBrowser 这个强工具，未来有对应场景可直接用
- **关键教训**：找到更强的工具但无法替换当前 ≠ 失败，是正常的进化路径。关键是记录下来，未来有对应场景直接用，不重复搜索。
- **CloakBrowser 正确用法**：`from cloakbrowser import launch; browser = launch()` → 非登录态的爬虫/反检测测试/匿名浏览。不需要用户登录态的场景直接用它。

**这次没有白学**：发现了 CloakBrowser 这个强工具，未来有对应场景可直接用

**配套规则（daily-self-evolution）**：
- 遇到错误 → 先判断：能替换吗？能升级到更好的吗？
- 修 3 次失败 → 触发进化模式，不再修补
- 进化结果必须入 fact_store，下次同类问题直接走替换路径
- **进化≠替换**：有时候找到更强的工具但当下无法替换（架构限制），这时记录工具价值和应用场景，等条件成熟再引入。这是正常的进化路径，不是失败。
...
- `references/evolution-candidates-2026-06-07.md` — **本次进化搜罗成果**：系统现状盘点 + 4个候选替代方案（Bifrost/You.com/CloakBrowser/n8n）+ 执行顺序建议
