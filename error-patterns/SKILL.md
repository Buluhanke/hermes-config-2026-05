...
## OmniRoute 安装踩坑（2026-07-10 更新 2026-07-21, 严重度5）
asar 打包不完整: DMG 里 app.asar 缺 ws-a972e7ffa40ff725 等 14 个包 → 重新下载 OmniRoute-x.x.x-arm64.dmg
better-sqlite3 ABI mismatch (2026-07-10): dist/node_modules/ 版本太旧 → rm -rf dist/node_modules/better-sqlite3 让 Node 向上找 root 的兼容版本

**2026-07-21 新发现 — DB probe 死循环**（比旧版更严重）：
- 症状：`/health` 和所有 `/v1/*` API 全部 500，Server 进程在运行（RSS 214MB）
- 日志循环：`[DB] Could not probe existing DB: out of memory` → rename corrupt → auto-restore → probe again → 死循环
- 根因：OmniRoute 内置 `better-sqlite3` probe 对某些 SQLite WAL 文件状态误判为 "out of memory"，触发了 DB 删除重建循环
- `rm dist/node_modules/better-sqlite3` 不可行（OmniRoute 自己的 `better-sqlite3` 实测 `import` 正常）
- `OMNIROUTE_SKIP_DB_HEALTHCHECK=1`（写入 `~/.omniroute/.env`）无效，仍触发 probe
- `Node --version`: v22.23.1, OmniRoute: 3.8.46
- 验证 OmniRoute 自身 better-sqlite3 正常：`cd ~/.local/lib/node_modules/omniroute && node -e "require('better-sqlite3')"`
- 日志路径：`~/.omniroute/logs/application/`
- 解决方向：重新编译 native 模块（`node-gyp rebuild`）或降级 Node 到 OmniRoute 兼容版本；或在 Next.js instrumentation 层禁用 DB 健康检查
